...
class Itoris_ViewMoreProducts_Helper_Form extends Itoris_ViewMoreProducts_Helper_Data {
			...
			if (get_class($fieldset) == 'Varien_Data_Form_Element_Fieldset') {
				foreach ($fieldset->getElements() as $element) {
					...
					if ($id = $element->getId()) {
						...
						$element->setUseParentValue($this->getSettings(true)->isParentValue($id, (!$checkStore && $checkWebsite)));
						$element->setUseScopeLabel($checkWebsite ? ($checkStore ? $this->__('Use Website') : $this->__('Use Default')) : null);
					}
				}
			}
		}
