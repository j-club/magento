...
class AW_Advancednewsletter_Adminhtml_SynchronizationController extends Mage_Adminhtml_Controller_Action {
    ...
    public function syncAction() {
        ...
        switch ($syncType) {
            ...
            case AW_Advancednewsletter_Block_Adminhtml_Synchronization::SYNC_TO_MAILCHIMP:
                ...
                Mage::getModel('advancednewsletter/cache')->saveCache(serialize($stores), 'aw_advancednewsletter_mailchimp_to_params');
            ...
            case AW_Advancednewsletter_Block_Adminhtml_Synchronization::SYNC_FROM_MAILCHIMP:
                ...
                Mage::getModel('advancednewsletter/cache')->saveCache(serialize($stores), 'aw_advancednewsletter_mailchimp_from_params');               
                $this->_addQueueDetailMessage();
                break;
    ...
    protected function _addQueueDetailMessage()
    {
        ...
        if(!$queued) {
            return Mage::getSingleton('adminhtml/session')->addNotice($this->_helper()->
                 __('Synchronization is disabled for all stores. Please check configuration settings.'));    
        }
        
         Mage::getSingleton('adminhtml/session')->addSuccess($this->_helper()->
                 __('Synchronization was queued for %d of total %d stores', $queued, $total));        
    ...
    public function newsletterSubscribersImportAction() {
        ...
        foreach ($newsletterSubscribersCollection as $subscriber) {
            ...
            switch ($subscriber->getStatus()) {
                case Mage_Newsletter_Model_Subscriber::STATUS_SUBSCRIBED: $newStatus = AW_Advancednewsletter_Model_Subscriber::STATUS_SUBSCRIBED;
                    break;
                case Mage_Newsletter_Model_Subscriber::STATUS_NOT_ACTIVE: $newStatus = AW_Advancednewsletter_Model_Subscriber::STATUS_NOTACTIVE;
                    break;
                case Mage_Newsletter_Model_Subscriber::STATUS_UNSUBSCRIBED: $newStatus = AW_Advancednewsletter_Model_Subscriber::STATUS_UNSUBSCRIBED;
                    break;
            }
            ...
            try {
                ...
                $sql = sprintf("INSERT IGNORE INTO %s (%s) VALUES('%s')", $subscriberTable, implode(',', array_keys($data)), implode("','", $data));
