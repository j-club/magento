...
class AW_Advancednewsletter_Model_Sync_Mailchimpclient implements AW_Advancednewsletter_Model_Sync_Interface {
    ...
    public static function getInstance($storeId) {
        ...
        if ($createNewInstance) {
            ...
            try {
                ...
                throw new AW_Advancednewsletter_Exception(Mage::helper('advancednewsletter')->__('Couldn\'t connect to MailChimp'));
            }
    ...
    public static function testConnection($params)
    {
        ...
        try {
              ...
              $client->call('chimpChatter', array(
                $instance->_keysValues[self::MAILCHIMP_APIKEY]             
              ));          
    ...
    public function subscribe($subscriber) {
        ...
        try {
             ...
             */
            $this->_client->call('listSubscribe', array(
    ...
    {      
        try {
            $this->_client->call('listInterestGroupings', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID]
            ));
    ...
    public function unsubscribeFromList($subscriber) {
        try {
             ...
             */
            $this->_client->call('listUnsubscribe', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID],
                $subscriber->getEmail(),
                false,
    ...
    protected function _listInterestGroupingAdd($groups, $groupingName = self::DEFAULT_GROUPING, $groupingType = 'checkboxes') {
        ...
        try {
             ...
             */
          return $this->_client->call('listInterestGroupingAdd', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID],
                $groupingName,
                $groupingType,
             ...
             $this->_logger()->log($this, sprintf("Failed to create grouping for list %s with error %s", $this->_keysValues[self::MAILCHIMP_LISTID], $e->getMessage()));
        }
    ...
    protected function _listInterestGroupAdd($groupName) {
        ...
        try {
             ...
             * listInterestGroupAdd(string apikey, string id, string group_name, int grouping_id)
             */
            $result = $this->_client->call('listInterestGroupAdd', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID],
                $groupName)
            );
    ...
    public function removeSegment($segmentCode) {
        try {
            $this->_client->call('listInterestGroupDel', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID],
                $segmentCode)
            );
    ...
    public function getChimpGroupings() {
        ...
        try {
            $chimpGroupings = $this->_client->call('listInterestGroupings', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID]
                    ));
        } catch (Exception $exc) {
    ...
    protected function _getListMembers($type='subscribed', $since=null, $pageNum=0, $pageSize=20) {
        ...
        try {
             ...
             */
            $members = $this->_client->call('listMembers', array(
                $this->_keysValues[self::MAILCHIMP_APIKEY],
                $this->_keysValues[self::MAILCHIMP_LISTID],
                $type,
                null,
     ...
     *   input - an array of up to 50 email addresses to get information for OR the "id"(s) for the member returned from listMembers, Webhooks, and Campaigns
     *   returns  array
     */
    ...
    protected function _getlistMemberInfo($email) {
        ...
        if (count($email)) {
            try {
                 ...
                 */
                $data = $this->_client->call('listMemberInfo', array(
                    $this->_keysValues[self::MAILCHIMP_APIKEY],
                    $this->_keysValues[self::MAILCHIMP_LISTID],
                    $email)
                );
    ...
    protected function setKeys($storeId) {
        ...
        if (!$this->_keysValues[self::MAILCHIMP_APIKEY] || !$this->_keysValues[self::MAILCHIMP_XMLRPC]) {
            throw new AW_Advancednewsletter_Exception(Mage::helper('advancednewsletter')->__('Apikey or xmlrpc url are inncorrect'));
        }        
    ...
    /*
     *   Subscribe batch persons to MailChimp list
     * 
     *  @param array $batch
     *  @param bool $doubleOptin - flag to control whether to send an opt-in confirmation email
     *  @param bool $updateExisting - flag to control whether to update members that are already subscribed to the list or to return an error
     *  @params bool $replaceInterests - flag to determine whether we replace the interest groups with the updated groups provided, 
    ...
    public function batchSubscribe($batch, $doubleOptin = FALSE, $updateExisting = TRUE, $replaceInterests = TRUE) {
         ...
         $interests = array();
        /* currently there is a BUG in mailchimp batch subscribe as groups are not created automatically */
          foreach($batch as $subscriber) {
              if(isset($subscriber['INTERESTS'])) {
                  $interests += explode(',', $subscriber['INTERESTS']);
              }           
          }          
          if(!empty($interests)) {            
              foreach($interests as $interest) {                  
                  $this->_listInterestGroupAdd($interest);                  
              }            
          }         
        /* */
        ...
        if (count($batch)) {
            try {
                 ...
                 */
                $data = $this->_client->call('listBatchSubscribe', array(
                    $this->_keysValues[self::MAILCHIMP_APIKEY],
                    $this->_keysValues[self::MAILCHIMP_LISTID],
                    $batch,
                    $doubleOptin, /* flag to control whether to send an opt-in confirmation email - defaults to true */
                    $updateExisting, /* flag to control whether to update members that are already subscribed to the list or to return an error, defaults to false (return error) */
                    $replaceInterests, /* flag to determine whether we replace the interest groups with the updated groups provided, 
                         * or we add the provided groups to the member's interest groups (optional, defaults to true) */
                        )
                );
    ...
    public function batchUnsubscribe($email, $deleteMembers = FALSE, $sendGoodbye = FALSE, $sendNotify = FALSE) {
        ...
        if (count($email)) {
            try {
                 ...
                 */
                $data = $this->_client->call('listBatchUnsubscribe', array(
                    $this->_keysValues[self::MAILCHIMP_APIKEY],
                    $this->_keysValues[self::MAILCHIMP_LISTID],
                    $email,
                    $deleteMembers,
