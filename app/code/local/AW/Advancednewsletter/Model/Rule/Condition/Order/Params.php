...
class AW_Advancednewsletter_Model_Rule_Condition_Order_Params extends Mage_Rule_Model_Condition_Abstract {
    ...
    public function asHtml() {
        if ($this->getAttribute() == 'sku') {
            $html = $this->getTypeElement()->getHtml() .
                    Mage::helper('advancednewsletter')->__("%s %s", $this->getAttributeElement()->getHtml(), $this->getValueElement()->getHtml()
            );
            if ($this->getId() != '1') {
    ...
    public function getValueElementType() {
        ...
        if ($this->getAttribute() == 'store' || $this->getAttribute() == 'category' || $this->getAttribute() == 'order_status')
            return 'select';
