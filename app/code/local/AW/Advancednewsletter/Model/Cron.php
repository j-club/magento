...
class AW_Advancednewsletter_Model_Cron {
    ...
    protected function _checkInstanceDuplicate($instances, $instanceForCheck) {
        ...
        foreach ($instances as $instance) {
            ...
            if (
                    $instanceKeys[AW_Advancednewsletter_Model_Sync_Mailchimpclient::MAILCHIMP_APIKEY] == $instanceForCheckKeys[AW_Advancednewsletter_Model_Sync_Mailchimpclient::MAILCHIMP_APIKEY]
                    && $instanceKeys[AW_Advancednewsletter_Model_Sync_Mailchimpclient::MAILCHIMP_LISTID] == $instanceForCheckKeys[AW_Advancednewsletter_Model_Sync_Mailchimpclient::MAILCHIMP_LISTID]
            ) {
                return true;
            }
        }
    ...
    public function syncFromMailchimp() {
        ...
        foreach ($instances as $instance) {
            ...
            $pageSize = ($syncFor === 'sync_statuses') ? self::SYNC_FROM_MAILCHIMP_PAGE_SIZE_2 : self::SYNC_FROM_MAILCHIMP_PAGE_SIZE;
            ...
            if (!($page === 'none')) {
                ...
                if (count($subscribers)) {
                    $this->_addRecordsToStore($subscribers, $storeId, $syncFor, AW_Advancednewsletter_Model_Subscriber::STATUS_SUBSCRIBED);
            ...
            if (!($page === 'none')) {
                ...
                if (count($unsubscribers)) {
                    $this->_addRecordsToStore($unsubscribers, $storeId, $syncFor, AW_Advancednewsletter_Model_Subscriber::STATUS_UNSUBSCRIBED);
        ...
        foreach ($syncFromParams as $key => $value) {
            if (
                    ($syncFromParams[$key]['subscr_page'] === 'none')
                    && ($syncFromParams[$key]['unsubscr_page'] === 'none')) {
     ...
     *  @param bool $includeName      Include FNAME && LNAME
     *  @param bool $includeInterests  
     *  @return array 
     */

    private function _getSubsBatch($subscribers, $includeNames, $includeInterests = FALSE) {
        $batch = array();
