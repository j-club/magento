...
class AW_Advancednewsletter_Model_Mysql4_Subscriber_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function removeSegment($code) {
        ...
        try {
            ...
            /* removed  is first */
            $sql = "UPDATE {$subscriber} SET `segments_codes` = SUBSTRING( `segments_codes` , ( POSITION( ',' IN `segments_codes` ) +1 ) )
                            WHERE `segments_codes` REGEXP '^{$code},'";
            $adapter->exec($sql);

            /* removed  is last */
            $sql = "UPDATE {$subscriber} SET 
                    `segments_codes` =  SUBSTRING( `segments_codes` , 1, (  LENGTH( `segments_codes` ) - length( '{$code}' ) - 1 ) )
                            WHERE `segments_codes` REGEXP ',{$code}$'";
            $adapter->exec($sql);
        } catch (Exception $exc) {
