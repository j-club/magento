...
class AW_Advancednewsletter_Model_Mysql4_Segment_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    {        
        $this->getSelect()
               ->columns(array('subscribers_count' => new Zend_Db_Expr("(SELECT(COUNT(IF(FIND_IN_SET(main_table.code, segments_codes), 
                   main_table.segment_id, NULL))) from {$this->getTable('advancednewsletter/subscriber')})") ));
