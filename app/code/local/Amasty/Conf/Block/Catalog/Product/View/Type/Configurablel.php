...
class Amasty_Conf_Block_Catalog_Product_View_Type_Configurablel extends Mage_Catalog_Block_Product_View_Type_Configurable
{
    ...
    public function getJsonConfig()
    {
        ...
        {
            $attr = $this->getThisAttribute($attributeId);
            ...
            if (in_array($attributeId, $productImagesAttributes) && $attr->getUseImage()){
                foreach ($attribute['options'] as $i => $option)
                {
                    ...
                    foreach($option['products'] as $product_id) {
                        $product = $this->_getUsedProductById($product_id);
     ...
     * @return bool
     */
    protected function _getUsedProductById($productId)
    {
     ...
     * @return mixed
     */
    public function getThisAttribute($attribute_id)
    {
        if (empty($this->_thisAttributes[$attribute_id])) {
            $this->_thisAttributes[$attribute_id] = Mage::getModel('amconf/attribute')
                ->load($attribute_id, 'attribute_id');
        }

        return $this->_thisAttributes[$attribute_id];
    }
}